...
const sessionHandler = this.SessionHandler();
  ...
  const sessionHandler = this.SessionHandler();
        return async (req, res) => {
            ...
            if(TWILIO_MODE=="ivr") {
                //const callSid = post.CallSid;
                const url = "https://" + req.headers["host"] + "/gather_default?phone="+phone+"&contractNum="+contractNum+"&userInput="+userInput+"&arrears="+arrears+"&fname="+fname+"&numMissed="+numMissed+"&daysSince="+daysSince;
